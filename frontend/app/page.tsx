...
import { getEvents, searchEvents, Event } from "@/lib/api";
...
export default function Home() {
    ...
    try {
      if (searchQuery.trim()) {
        ...
        if (selectedCategory !== "All") {
          setEvents(data.filter(e => e.category === selectedCategory));
        } else {
          setEvents(data);
        }
      } else {
        const data = await getEvents(selectedCategory, sortBy);
        setEvents(data);
      }
      
      if (!searchQuery.trim() && selectedCategory === "All") {
        const [soonData, trendingData, recData] = await Promise.all([
          getEvents("All", "soonest", "happeningSoon"),
          getEvents("All", "popular", "trending"),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/ai/events/recommendations`, {
  ...
  }, [searchQuery, selectedCategory, sortBy, session]);
      ...
      <div className="bg-[#121215] border-b border-[#27272a] py-16 md:py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <h2 className="text-4xl md:text-6xl font-extrabold tracking-tight text-white mb-6">
            Discover <span className="text-[#e50914]">Local Events</span> Near You
          </h2>
          <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto mb-10">
            Find events worth attending. Connect with your community, learn new skills, and experience local culture.
          </p>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
              <div key={n} className="h-[400px] bg-[#18181b] animate-pulse rounded-xl border border-[#27272a]" />
            ))}
          </div>
        ) : error ? (
          <div className="py-24 text-center max-w-md mx-auto">
            <h3 className="text-2xl font-bold text-white mb-2">Unable to load events</h3>
            <p className="text-gray-400 mb-8">Please check your connection and try again.</p>
            <button onClick={fetchEvents} className="px-6 py-2 bg-[#e50914] text-white font-medium rounded-md hover:bg-[#b80710] transition-colors">
              Retry
            </button>
          </div>
        ) : events.length === 0 ? (
          ...
          <div className="space-y-16">
            ...
            {!searchQuery && selectedCategory === "All" && happeningSoon.length > 0 && (
              <section>
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-2 h-8 bg-green-500 rounded-full"></div>
                  <h3 className="text-2xl font-bold text-white">Happening Soon</h3>
                </div>
                <div className="border-t border-[#27272a] pt-6">
                  <EventGrid events={happeningSoon.slice(0, 4)} />
                </div>
              </section>
            )}
