...
export function EventCard({ event }: EventCardProps) {
  ...
  const formattedEndTime = eventEndDate.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  return (
    <Card className="flex flex-col h-full hover:shadow-2xl transition-all duration-300 relative bg-[#18181b] text-white border-[#27272a] hover:bg-[#27272a]/50 overflow-hidden group">
      ...
      <CardHeader
        className={`relative z-10 pointer-events-none pb-2 ${
          event.image_url ? "pt-0" : ""
        }`}
      >
        <div className="flex justify-between items-start mb-3">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            {/* STATUS */}
            <div
              className={`px-2 py-1 rounded-md border text-xs font-semibold flex items-center gap-1.5 ${badgeClasses}`}
            >
              <span
        ...
        <CardTitle className="line-clamp-2 text-xl font-extrabold mb-2 text-white">
          {event.title}
        </CardTitle>
      </CardHeader>
      ...
      <CardFooter className="pt-2 pb-6 flex justify-between items-center relative z-20">
        ...
        <Link href={`/events/${event.id}`}>
          <Button
            variant="outline"
            className="bg-transparent border-[#27272a] text-white hover:bg-[#e50914] hover:text-white hover:border-[#e50914] transition-colors"
          >
            View Event
          </Button>
        </Link>

        {/* RSVP ONLY WHEN PUBLISHED */}
        {canRSVP && (
          <Link href={`/events/${event.id}`}>
