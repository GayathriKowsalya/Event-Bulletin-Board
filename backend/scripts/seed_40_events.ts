...
async function seed() {
  ...
  const { data: users, error: usersErr } = await supabase.from('profiles').select('id, role').limit(50);
  if (usersErr) throw usersErr;
  
  let adminUserId = users.find(u => u.role === 'admin')?.id;
  if (!adminUserId) {
    if (users.length > 0) adminUserId = users[0].id;
    else throw new Error("No users found in database to act as event creator.");
  }
  ...
  for (let i = 0; i < allEvents.length; i++) {
    ...
    if (status === 'active' || status === 'expired') {
        const rsvpCount = Math.min(
            ...
            Math.floor(Math.random() * Math.min(capacity, 50)) + 5
        );
        ...
        if (rsvpInserts.length > 0) {
            await supabase.from('event_registrations').insert(rsvpInserts);
        }
    }
  }
  ...
  const { count: cbeCount } = await supabase.from('events').select('*', { count: 'exact', head: true }).ilike('location', '%Coimbatore%');
  const { count: tryCount } = await supabase.from('events').select('*', { count: 'exact', head: true }).ilike('location', '%Trichy%');
  const { count: cheCount } = await supabase.from('events').select('*', { count: 'exact', head: true }).ilike('location', '%Chennai%');
  ...
  const { data: statuses } = await supabase.from('events').select('status');
